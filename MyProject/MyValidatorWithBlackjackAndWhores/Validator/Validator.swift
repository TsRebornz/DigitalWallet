...
public class Validator {
    ...
    /**
     ...
     - returns: No return value.
     */
    public func validateField(field: ValidatableField, callback: (_ error:ValidationError?) -> Void){
    ...
    public func unregisterField(field:ValidatableField) {
        validations.removeValueForKey(key: field)
        errors.removeValueForKey(key: field)
    }
    ...
    public func validate(delegate:ValidationDelegate) {
        ...
        if errors.isEmpty {
            ...
            var errorTupleArr = [(Validatable, ValidationError)]()
            for error in self.errors {
                let errorTuple : (Validatable, ValidationError) = (fields[(error.value as ValidationError).field]!, error.value)
                errorTupleArr += [errorTuple]
            }
            delegate.validationFailed(errors: errorTupleArr )
        }
    ...
    /**
     ...
     - returns: No return value.
     */
    public func validate(callback:(_ errors:[(Validatable, ValidationError)])->Void) -> Void {
